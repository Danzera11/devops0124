    ...
    stages {
        ...
        stage('Build Images') {
            steps {
                script {
                    // Nome das imagens
                    def imageNameWeb = 'danzera11/web'
                    def imageNameDB = 'danzera11/db'
                    def imageNameNginx = 'danzera11/nginx'

                    // Executa o build das imagens com os respectivos Dockerfiles
                    docker.withRegistry('https://registry.hub.docker.com', 'dockerhubdanzera') {
